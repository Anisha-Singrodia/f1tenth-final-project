...
class Workspace:
    ...
    def setup(self):
        ...
        # dataset
        self.train_dataset = F1TENTH_Dataset(self.cfg.train_data_path, history_len=self.cfg.model.history_len)
        self.eval_dataset = F1TENTH_Dataset(self.cfg.eval_data_path, history_len=self.cfg.model.history_len)
        ...
        if self.cfg.model.name == 'MLP':
            self.model = MLPdynamics(self.cfg.model).to(self.device)
        elif self.cfg.model.name == 'Transformer':
            self.model = Transformerdynamics(self.cfg.model).to(self.device)
        else:
